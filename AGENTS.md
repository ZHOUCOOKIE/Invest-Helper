...
- 抽取语义不可漂移：`assets` 最终形态必须为对象数组；无标的用 `NoneAny` 哨兵对象；`content_kind=library` 时仅入库知识库语义，不进入资产观点列表。
- Library 边界强约束（A2）：`library_entry` 为入库唯一有效入口，且必须满足：
  - `confidence >= 70`
  - `tags` 长度 `1..2`
  - `tags` 枚举固定为 `macro/industry/thesis/strategy/risk/events`
  - `content_kind=library` 且 `library_entry` 无效/缺失时，必须 normalize 降级为 `content_kind=asset`（不走失败兜底）
- `content_kind=library` 时 `assets` 必须为 `[NoneAny]`，`asset_views` 最终必须被清空为 `[]`（模型乱输出也要 normalize 清空并留 meta 证据）。
- 向后兼容：历史 `approved/rejected` 记录不得回写重算；读取旧记录缺字段时必须宽容回退（`content_kind->asset`，`library_entry->null`）；旧记录若仍含 `library_tags` 必须忽略且不得报错，不得改变其状态展示。
