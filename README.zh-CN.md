...
系统将非结构化帖子转成结构化信号，并支持按 `profile_id + digest_date + version` 做版本化回放。

## 产品价值

- 所有关键输出可回链证据来源，便于审计与复盘。
- 将帖子流转化为标准化 `asset_views`，提升可消费性。
- 支持 profile 维度的观点聚合与版本化 Digest。
- 提供可落地的本地运行与统一验收流程。
...
- 自动审核逻辑：
  - 非 library 且 `hasview=0` 自动拒绝
  - 非 library 且满足置信度路径走阈值审核（`70`）
  - `islibrary=1` 自动通过（`library_flag`）
- 证据链贯通 `raw_posts -> post_extractions -> kol_views -> daily_digests`。
- Digest 支持按 `profile_id + digest_date` 递增版本并回放。

## 产品界面与技术栈

- API：FastAPI + SQLAlchemy + Alembic
- Web：Next.js（dashboard/review/profile/digest）
