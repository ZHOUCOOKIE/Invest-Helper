...
InvestPulse 是一个面向投研与交易工作流的投资信号看板系统，核心强调证据可追溯与摘要可回放。系统会把来源帖子转成结构化观点，支持审核流程，并提供日报与周报回放能力。
...
- 当前 library 分支仍保留严格的测试型约束：`library_entry.summary` 必须等于 `测试`。
- `hasview=0` 会自动拒绝。
...
- 周报 AI 输入按天聚合，当前提示词重点放在日内到未来 `1w/1m` 的影响，并要求短线交易观察按作者归类。
...
- Profile 表与服务端规则加载已存在，但公开的 profile 管理 API 还未暴露。
