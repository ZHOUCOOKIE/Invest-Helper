...
- 规范输出顺序：
  - 顶层：`as_of, source_url, islibrary, hasview, asset_views, library_entry`（`extracted_json` 可在末尾追加 `meta`）
  - `asset_views[*]`：`symbol, market, stance, horizon, confidence, summary`
- `market/stance/horizon` 使用严格枚举。
- 自动审核逻辑：
  - `hasview=0` 自动拒绝
  - `hasview=1` 且满足置信度路径走阈值审核（`80`）
  - 拒绝原因键统一为 `meta.auto_review_reason`（历史 `auto_reject_*` 已移除）
- 解析失败语义：
  - 模型返回内容但解析失败时，记录仍会以 `pending` 落库
  - 失败原因写入 `last_error`/parse meta，并在进度/重试分类中按 failed 语义处理
- `parsed_model_output` 使用 DB 有序 `JSON` 存储（非 `JSONB`），用于稳定回放/调试字段顺序。
