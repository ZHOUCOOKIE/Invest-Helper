...
- `Daily Digest`: 按 `profile_id + digest_date` 唯一行覆盖生成的日度输出（当前 API 使用系统 profile `id=1`）。
- `Weekly Digest`: 按 `profile_id + report_kind + anchor_date` 唯一行覆盖生成的周度输出（当前 API 使用系统 profile `id=1`）。
- `Replay`: 按 `date`（系统 profile）或 `digest_id` 读取 digest。
- `Traceability`: 从 digest/view 回链到 `source_url`, `raw_posts`, `post_extractions` 的能力。
- `Portfolio Advice`: 基于用户持仓文本与证据引用做的组合建议接口（`POST /portfolio/advice`）。
